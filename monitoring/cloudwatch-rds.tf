...
resource "aws_cloudwatch_metric_alarm" "db_cpu_critical" {
  alarm_name          = "${var.environment_name}_database_cpu--critical"
...
resource "aws_cloudwatch_metric_alarm" "db_cpu_warning" {
  alarm_name          = "${var.environment_name}_database_cpu--warning"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = 1
  metric_name         = "CPUUtilization"
  namespace           = "AWS/RDS"
...
resource "aws_cloudwatch_metric_alarm" "db_cpu_credit_balance" {
  alarm_name          = "${var.environment_name}_database_cpu_credit_balance--warning"
...
resource "aws_cloudwatch_metric_alarm" "db_disk_queue_depth" {
  alarm_name          = "${var.environment_name}_database_disk_queue_depth--warning"
...
resource "aws_cloudwatch_metric_alarm" "db_free_storage_space_critical" {
  alarm_name          = "${var.environment_name}_database_free_storage_space--critical"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 1
...
resource "aws_cloudwatch_metric_alarm" "db_free_storage_space_warning" {
  alarm_name          = "${var.environment_name}_database_free_storage_space--warning"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 1
  metric_name         = "FreeStorageSpace"
  namespace           = "AWS/RDS"
...
resource "aws_cloudwatch_metric_alarm" "db_burst_balance" {
  alarm_name          = "${var.environment_name}_database_burst_balance--warning"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 1
...
resource "aws_cloudwatch_metric_alarm" "db_freeable_memory" {
  alarm_name          = "${var.environment_name}_database_freeable_memory--warning"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = 1
...
resource "aws_cloudwatch_metric_alarm" "db_connections" {
  alarm_name          = "${var.environment_name}_database_connections--warning"
  comparison_operator = "GreaterThanThreshold"
  evaluation_periods  = 1
  ...
  statistic           = "Average"
  threshold           = 600
  alarm_description   = "Average database connections over 600, check for database"
  alarm_actions       = [local.sns_alarm_notification_arn]
  ok_actions          = [local.sns_alarm_notification_arn]
...
resource "aws_cloudwatch_metric_alarm" "db_anomalous_connections" {
  alarm_name          = "${var.environment_name}_database_anomalous_connections--critical"
